...
import { CreateContentUnorderedlistDto, UpdateContentUnorderedlistDto } from "../dto";

@Injectable()
export class ContentUnorderedlistService {
  constructor(
    @InjectRepository(ContentUnorderedlistEntity)
  ...
  async findAll(options: PaginationOptionsDto): Promise<PaginatedResponseDto<ContentUnorderedlistEntity>> {
  ...
  async findOne(id: number): Promise<ContentUnorderedlistEntity> {
    ...
    if (!entity) throw new Error(`Content unorderedlist ${id} not found`);
    return entity;
  }
