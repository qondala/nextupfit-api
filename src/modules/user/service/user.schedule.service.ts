...
export class UserScheduleService {
  ...
  ): Promise<PaginatedResponseDto<UserScheduleEntity>> {
    const { page = 1, limit = 10 } = options;
    const skip = (page - 1) * limit;

    const [userSchedules, total] =
      await this.userScheduleRepository.findAndCount({
        where: { userId },
        skip,
        take: limit,
        order: {
  ...
  async update(
    id: number,
    updateUserScheduleDto: UpdateUserScheduleDto,
  ): Promise<UserScheduleEntity> {
