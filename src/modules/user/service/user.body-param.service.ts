...
export class UserBodyParamService {
  ...
  async create(
    createUserBodyParamDto: CreateUserBodyParamDto,
  ): Promise<UserBodyParamEntity> {
    const userBodyParam = this.userBodyParamRepository.create(
  ...
  ): Promise<PaginatedResponseDto<UserBodyParamEntity>> {
  ...
  async findOne(id: number): Promise<UserBodyParamEntity> {
    const userBodyParam = await this.userBodyParamRepository.findOne({
      where: { id },
    });
    if (!userBodyParam) {
  ...
  async getUserBodyParams(
    userId: number,
  ): Promise<PaginatedResponseDto<UserBodyParamEntity>> {
    const userBodyParams = await this.userBodyParamRepository.find({
      where: { userId },
    });
    return {
  ...
  async update(
    id: number,
    updateUserBodyParamDto: UpdateUserBodyParamDto,
  ): Promise<UserBodyParamEntity> {
