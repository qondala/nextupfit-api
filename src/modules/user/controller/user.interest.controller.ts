...
export class UserInterestController {
  ...
  async create(
    @Body() createDto: CreateUserInterestDto,
  ): Promise<DetailsUserInterestDto> {
    const item = await this.userInterestService.create(createDto);
    const interest = await this.userInterestService.getUserInterestComposite(
      item.id,
      item.interestType,
    );
    return {
  ...
  async createMany(
    ...
    return Promise.all(
      userInterests.map(async (record) => {
        const interest =
          await this.userInterestService.getUserInterestComposite(
            record.id,
            record.interestType,
          );
        return {
  ...
  async findAll(
    ...
    const paginatedUserInterests = await this.userInterestService.findAll(
      userId,
      { page, limit },
    );
    return {
      items: await Promise.all(
        paginatedUserInterests.items.map(async (record) => {
          const interest =
            await this.userInterestService.getUserInterestComposite(
              record.id,
              record.interestType,
  ...
  async findOne(
    ...
    if (!item)
      throw new NotFoundException(`User interest with ID ${id} not found`);
    const interest = await this.userInterestService.getUserInterestComposite(
      item.id,
      item.interestType,
    );
