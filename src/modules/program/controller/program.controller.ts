...
export class ProgramController {
  ...
  async findAll(
    ...
    const details = await Promise.all(
      result.items.map(
        async (program) => await this.extractProgramDetails(program),
      ),
    );
   ...
   */
  private async extractProgramDetails(
    ...
    const subscriptionPlansFound =
      await this.programSubscriptionPlanService.findAll(
        subscriptionPlanCriteria,
      );
