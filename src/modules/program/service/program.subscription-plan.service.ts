...
export class ProgramSubscriptionPlanService {
  ...
  async create(
    createProgramSubscriptionPlanDto: CreateProgramSubscriptionPlanDto,
  ): Promise<ProgramSubscriptionPlanEntity> {
    const programSubscriptionPlan = this.repository.create(
      createProgramSubscriptionPlanDto,
    );
  ...
  async findAll(
  ...
  ): Promise<PaginatedResponseDto<ProgramSubscriptionPlanEntity>> {
    ...
    if (criteria.planName) {
      queryBuilder.andWhere(
        "programSubscriptionPlan.planName ILIKE :planName",
        { planName: `%${criteria.planName}%` },
      );
    }
    if (criteria.priceHigherThan) {
      queryBuilder.andWhere(
        "programSubscriptionPlan.price >= :priceHigherThan",
        { priceHigherThan: criteria.priceHigherThan },
      );
    }
    if (criteria.priceLessThan) {
      queryBuilder.andWhere("programSubscriptionPlan.price <= :priceLessThan", {
        priceLessThan: criteria.priceLessThan,
      });
    }
    if (criteria.periodicity) {
      queryBuilder.andWhere(
        "programSubscriptionPlan.periodicity = :periodicity",
        { periodicity: criteria.periodicity },
      );
    }
    if (criteria.description) {
      queryBuilder.andWhere(
        "programSubscriptionPlan.description ILIKE :description",
        { description: `%${criteria.description}%` },
      );
    }
    if (criteria.programId) {
      queryBuilder.andWhere("programSubscriptionPlan.programId = :programId", {
        programId: criteria.programId,
      });
    }
    if (criteria.contentId) {
      queryBuilder.andWhere("programSubscriptionPlan.contentId = :contentId", {
        contentId: criteria.contentId,
      });
    }
    if (criteria.active) {
      queryBuilder.andWhere("programSubscriptionPlan.active = :active", {
      ...
      switch (criteria.orderBy) {
        ...
        case ProgramFindSubscriptionPlanOrderByEnum.attendeesCount:
          queryBuilder.addOrderBy(
            "programSubscriptionPlan.attendeesCount",
            "ASC",
          );
          break;
  ...
  async getAllSubscriptionPlansOfProgram(
    programId: number,
  ): Promise<ProgramSubscriptionPlanEntity[]> {
  ...
  async findOne(id: number): Promise<ProgramSubscriptionPlanEntity> {
    const programSubscriptionPlan = await this.repository.findOne({
      where: { id },
    });
  ...
  async update(
    id: number,
    updateProgramSubscriptionPlanDto: UpdateProgramSubscriptionPlanDto,
  ): Promise<ProgramSubscriptionPlanEntity> {
