...
export class ProgramWorkoutNutrientBurnService {
  ...
  async create(
    ...
    const programWorkoutNutrientBurn =
      this.programWorkoutNutrientBurnRepository.create(
        createProgramWorkoutNutrientBurnDto,
      );
    return await this.programWorkoutNutrientBurnRepository.save(
      programWorkoutNutrientBurn,
    );
  }

  async findAll(
    options: PaginationOptionsDto,
  ): Promise<PaginatedResponseDto<ProgramWorkoutNutrientBurnEntity>> {
    const { page = 1, limit = 10 } = options;
    const skip = (page - 1) * limit;

    const [items, totalItems] =
      await this.programWorkoutNutrientBurnRepository.findAndCount({
        skip,
        take: limit,
        order: {
          createdAt: "DESC",
  ...
  async findOne(id: number): Promise<ProgramWorkoutNutrientBurnEntity> {
    const programWorkoutNutrientBurn =
      await this.programWorkoutNutrientBurnRepository.findOne({
        where: { id },
      });
  ...
  async remove(id: number): Promise<void> {
    const programWorkoutNutrientBurn = await this.findOne(id);
    await this.programWorkoutNutrientBurnRepository.remove(
      programWorkoutNutrientBurn,
    );
  }
