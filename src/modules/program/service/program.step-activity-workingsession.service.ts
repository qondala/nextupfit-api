...
export class ProgramStepActivityWorkingsessionService {
  ...
  async create(body: CreateProgramStepActivityWorkingsessionDto): Promise<ProgramStepActivityWorkingsessionEntity> {
  ...
  async findAll(
    ...
    pagination?: PaginationOptionsDto): Promise<PaginatedResponseDto<ProgramStepActivityWorkingsessionEntity>> {


    const query = this.repository.createQueryBuilder("workingsessionQuery");
    
    if (criteria.workingSessionId) {
      query.andWhere("workingsessionQuery.id = :workingSessionId", { workingSessionId: criteria.workingSessionId });
    }

    if (criteria.baseWorkoutId) {
      query.andWhere("workingsessionQuery.baseWorkoutId = :baseWorkoutId", { baseWorkoutId: criteria.baseWorkoutId });
    }

    if (criteria.search) {
      const searchSQL =  "(workingsessionQuery.title ILIKE :search OR workingsessionQuery.description ILIKE :search)";
      query.andWhere(searchSQL, { search: `%${criteria.search}%` });
    }

    if (criteria.gymId) {
      query.andWhere("workingsessionQuery.gymId = :gymId", { gymId: criteria.gymId });
    }

    if (criteria.programId) {
      query.andWhere("workingsessionQuery.programId = :programId", { programId: criteria.programId });
    }

    if (criteria.programStepId) {
      query.andWhere("workingsessionQuery.programStepId = :programStepId", { programStepId: criteria.programStepId });
    }

    if (criteria.programStepActivityId) {
      query.andWhere("workingsessionQuery.programStepActivityId = :programStepActivityId", { programStepActivityId: criteria.programStepActivityId });
    }

    if (criteria.ownerUserId) {
      query.andWhere("workingsessionQuery.ownerUserId = :ownerUserId", { ownerUserId: criteria.ownerUserId });
    }

    if (criteria.ownerManagerId) {
      query.andWhere("workingsessionQuery.ownerManagerId = :ownerManagerId", { ownerManagerId: criteria.ownerManagerId });
    }

    if (criteria.status) {
      query.andWhere("workingsessionQuery.status = :status", { status: criteria.status });
    }

    if (criteria.attendeesCount) {
      query.andWhere("workingsessionQuery.attendeesCount = :attendeesCount", { attendeesCount: criteria.attendeesCount });
    }

    if (criteria.viewsCount) {
      query.andWhere("workingsessionQuery.viewsCount = :viewsCount", { viewsCount: criteria.viewsCount });
    }

    if (criteria.ratingsAvg) {
      query.andWhere("workingsessionQuery.ratingsAvg = :ratingsAvg", { ratingsAvg: criteria.ratingsAvg });
    }

    if (criteria.ratingsCount) {
      query.andWhere("workingsessionQuery.ratingsCount = :ratingsCount", { ratingsCount: criteria.ratingsCount });
    }

    if (criteria.difficultyLevel) {
      query.andWhere("workingsessionQuery.difficultyLevel = :difficultyLevel", { difficultyLevel: criteria.difficultyLevel });
    }
  ...
  async update(id: number, body: UpdateProgramStepActivityWorkingsessionDto): Promise<ProgramStepActivityWorkingsessionEntity> {
