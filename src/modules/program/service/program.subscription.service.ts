...
import { ProgramSubscriptionEntity } from "../entity";
import {
...
export class ProgramSubscriptionService {
  ...
  async create(
    createProgramSubscriptionDto: CreateProgramSubscriptionDto,
  ): Promise<ProgramSubscriptionEntity> {
    const programSubscription = this.repository.create(
      createProgramSubscriptionDto,
    );
  ...
  async findAll(
    criteria: ProgramFindCriteriaSubscriptionDto,
    pagination?: PaginationOptionsDto,
  ): Promise<PaginatedResponseDto<ProgramSubscriptionEntity>> {
    ...
    if (criteria.programId) {
      queryBuilder.andWhere("programSubscription.programId = :programId", {
    ...
    if (criteria.subscriberUserId) {
      queryBuilder.andWhere(
        "programSubscription.subscriberUserId = :subscriberUserId",
        { subscriberUserId: criteria.subscriberUserId },
      );
    }

    if (criteria.programSubscriptionPlanId) {
      queryBuilder.andWhere(
        "programSubscription.programSubscriptionPlanId = :programSubscriptionPlanId",
        { programSubscriptionPlanId: criteria.programSubscriptionPlanId },
      );
    }

    if (criteria.status) {
      queryBuilder.andWhere("programSubscription.status = :status", {
  ...
  async findByUserIdAndProgramId(
    ...
    return await this.repository.find({
      where: { subscriberUserId: userId, programId },
    });
  }

  async findByUserIdAndSubscriptionPlanId(
    userId: number,
    programSubscriptionPlanId: number,
  ): Promise<ProgramSubscriptionEntity[]> {
    return await this.repository.find({
      where: { subscriberUserId: userId, programSubscriptionPlanId },
    });
  }
  ...
  async findOne(id: number): Promise<ProgramSubscriptionEntity> {
    const programSubscription = await this.repository.findOne({
      where: { id },
    });
  ...
  async update(
    id: number,
    updateProgramSubscriptionDto: UpdateProgramSubscriptionDto,
  ): Promise<ProgramSubscriptionEntity> {
