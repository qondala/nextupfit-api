...
export class ProgramStepActivityWorkingsessionWorkoutService {
  ...
  async create(
    body: CreateProgramStepActivityWorkingsessionWorkoutDto,
  ): Promise<ProgramStepActivityWorkingsessionWorkoutEntity> {
  ...
  async findAll(
    ...
    PaginatedResponseDto<ProgramStepActivityWorkingsessionWorkoutEntity>
  > {
    ...
    if (criteria.workingSessionId) {
      queryBuilder.andWhere(
        "workoutQuery.workingSessionId = :workingSessionId",
        { workingSessionId: criteria.workingSessionId },
      );
    }
    if (criteria.baseWorkoutId) {
      queryBuilder.andWhere("workoutQuery.baseWorkoutId = :baseWorkoutId", {
        baseWorkoutId: criteria.baseWorkoutId,
      });
    }
    if (criteria.title) {
      queryBuilder.andWhere("workoutQuery.title ILIKE :title", {
        title: `%${criteria.title}%`,
      });
    }
    if (criteria.description) {
      queryBuilder.andWhere("workoutQuery.description ILIKE :description", {
        description: `%${criteria.description}%`,
      });
    }
    if (criteria.gymId) {
      queryBuilder.andWhere("workoutQuery.gymId = :gymId", {
        gymId: criteria.gymId,
      });
    }
    if (criteria.programId) {
      queryBuilder.andWhere("workoutQuery.programId = :programId", {
        programId: criteria.programId,
      });
    }
    if (criteria.programStepId) {
      queryBuilder.andWhere("workoutQuery.programStepId = :programStepId", {
        programStepId: criteria.programStepId,
      });
    }
    if (criteria.programStepActivityId) {
      queryBuilder.andWhere(
        "workoutQuery.programStepActivityId = :programStepActivityId",
    ...
    if (criteria.ownerUserId) {
      queryBuilder.andWhere("workoutQuery.ownerUserId = :ownerUserId", {
        ownerUserId: criteria.ownerUserId,
      });
    }
    if (criteria.ownerManagerId) {
      queryBuilder.andWhere("workoutQuery.ownerManagerId = :ownerManagerId", {
        ownerManagerId: criteria.ownerManagerId,
      });
    }
    if (criteria.status) {
      queryBuilder.andWhere("workoutQuery.status = :status", {
        status: criteria.status,
      });
    }
    if (criteria.attendeesCount) {
      queryBuilder.andWhere("workoutQuery.attendeesCount = :attendeesCount", {
        attendeesCount: criteria.attendeesCount,
      });
    }
    if (criteria.viewsCount) {
      queryBuilder.andWhere("workoutQuery.viewsCount = :viewsCount", {
        viewsCount: criteria.viewsCount,
      });
    }
    if (criteria.ratingsAvg) {
      queryBuilder.andWhere("workoutQuery.ratingsAvg = :ratingsAvg", {
        ratingsAvg: criteria.ratingsAvg,
      });
    }
    if (criteria.ratingsCount) {
      queryBuilder.andWhere("workoutQuery.ratingsCount = :ratingsCount", {
  ...
  async findPrevious(
    ...
    const previousWorkout = await this.repository.findOne({
      where: {
        workingSessionId: workout.workingSessionId,
        position: LessThan(workout.position),
      },
      order: { position: "DESC" },
    });
  ...
  async update(
    id: number,
    body: UpdateProgramStepActivityWorkingsessionWorkoutDto,
  ): Promise<ProgramStepActivityWorkingsessionWorkoutEntity> {
