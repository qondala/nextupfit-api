...
export class GymOpenDayService {
  ...
  async create(createDto: CreateGymOpenDayDto): Promise<GymOpenDayEntity> {
    if (await this.isOpeningDay(createDto.gymId, createDto.day)) {
      throw new ConflictException(
        `${createDto.day} is already an opening day in Gym ${createDto.gymId}`,
      );
    }
  ...
  async update(
    id: number,
    updateDto: UpdateGymOpenDayDto,
  ): Promise<GymOpenDayEntity> {
    if (await this.isOpeningDay(updateDto.gymId, updateDto.day)) {
      throw new ConflictException(
        `${updateDto.day} is already an opening day in Gym ${updateDto.gymId}`,
      );
    }

    await this.gymOpenDayRepository.update({ id }, updateDto);
