...
export class GymMembershipService {
  ...
  async create(
    createDto: CreateGymMembershipDto,
    ...
    if (
      await this.isMemberWithPlanId(
        createDto.memberUserId,
        createDto.gymId,
        createDto.gymMembershipPlanId,
      )
    ) {
      throw new ConflictException(
        `User ${createDto.memberUserId} is already member of Gym ${createDto.gymId} in the plan ${createDto.gymMembershipPlanId}`,
      );
    }
  ...
  async getAllMembershipsOfGym(
    ...
    const queryBuilder = this.gymMembershipRepository
  ...
  async update(
    id: number,
    updateDto: UpdateGymMembershipDto,
  ): Promise<GymMembershipEntity> {
    await this.gymMembershipRepository.update({ id }, updateDto);
