...
export class BaseWorkoutHowtoPerformStepController {
  constructor(
    private readonly baseWorkoutHowtoPerformStepService: BaseWorkoutHowtoPerformStepService,
  ) {}
  ...
  @ApiResponse({
    ...
    description:
      "The workout howto perform step has been successfully created.",
  ...
  async create(
    ...
    try {
      return (await this.baseWorkoutHowtoPerformStepService.create(
        createBaseWorkoutHowtoPerformStepDto,
      )) as unknown as DetailsBaseWorkoutHowtoPerformStepDto;
    } catch (error) {
      if (error.code === "23505") {
        // PostgreSQL unique violation error code
        throw new BadRequestException(
          "Workout howto perform step with this code already exists",
        );
      }
      throw error;
    }
  ...
  @ApiOperation({
    summary:
      "Search workout howto perform steps by query string with pagination",
    description:
      "Search workout howto perform steps by query string with pagination",
  ...
  async findByCode(
    ...
    if (!step) {
      throw new NotFoundException(
        `Workout howto perform step with code '${code}' not found`,
      );
    }
    return step;
  }
  ...
  async findOne(
    ...
    if (!step) {
      throw new NotFoundException(
        `Workout howto perform step with ID ${id} not found`,
      );
    }
    return step;
  }
  ...
  async update(
    ...
    try {
      const step = (await this.baseWorkoutHowtoPerformStepService.update(
        id,
        updateBaseWorkoutHowtoPerformStepDto,
      )) as unknown as DetailsBaseWorkoutHowtoPerformStepDto;
      if (!step) {
