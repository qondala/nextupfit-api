...
export class BaseUnitController {
  ...
  async create(
    @Body() createBaseUnitDto: CreateBaseUnitDto,
  ): Promise<DetailsBaseUnitDto> {
    try {
      return (await this.baseUnitService.create(
        createBaseUnitDto,
      )) as unknown as DetailsBaseUnitDto;
    } catch (error) {
      if (error.code === "23505") {
  ...
  async update(
    ...
    try {
      const unit = (await this.baseUnitService.update(
        id,
        updateBaseUnitDto,
      )) as unknown as DetailsBaseUnitDto;
      if (!unit) {
