...
export class BaseWorkoutRecommendedRepetitionController {
  constructor(
    private readonly baseWorkoutRecommendedRepetitionService: BaseWorkoutRecommendedRepetitionService,
  ) {}
  ...
  @ApiResponse({
    ...
    description:
      "The workout recommended repetition has been successfully created.",
  ...
  async create(
    ...
    return (await this.baseWorkoutRecommendedRepetitionService.create(
      createBaseWorkoutRecommendedRepetitionDto,
    )) as unknown as DetailsBaseWorkoutRecommendedRepetitionDto;
  }
  ...
  async findOne(
    ...
    const repetition =
      await this.baseWorkoutRecommendedRepetitionService.findOne(id);
    if (!repetition) {
      throw new NotFoundException(
        `Workout recommended repetition with ID ${id} not found`,
      );
    }
    return repetition;
  }
  ...
  async update(
    ...
    const repetition =
      (await this.baseWorkoutRecommendedRepetitionService.update(
        id,
        updateBaseWorkoutRecommendedRepetitionDto,
      )) as unknown as DetailsBaseWorkoutRecommendedRepetitionDto;
    if (!repetition) {
  ...
  @ApiResponse({
    ...
    description:
      "The workout recommended repetition has been successfully deleted",
  })
  @ApiResponse({
    status: HttpStatus.NOT_FOUND,
    description: "Workout recommended repetition not found",
  ...
  async remove(@Param("id", ParseIntPipe) id: number): Promise<void> {
    const result =
      await this.baseWorkoutRecommendedRepetitionService.remove(id);
    if (!result) {
      throw new NotFoundException(
        `Workout recommended repetition with ID ${id} not found`,
      );
