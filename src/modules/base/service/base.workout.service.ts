...
export class BaseWorkoutService {
  ...
  async create(
    createBaseWorkoutDto: CreateBaseWorkoutDto,
  ): Promise<BaseWorkoutEntity> {
  ...
  async findAll(
    ...
    const queryBuilder =
      this.baseWorkoutRepository.createQueryBuilder("workout");

    const defaultWorkout =
      "workout.createdByUserId IS NULL OR workout.createdByUserId = 0";
    if (createdByUserId) {
      queryBuilder.where(
        `${defaultWorkout} OR workout.createdByUserId = :createdByUserId`,
        { createdByUserId },
      );
    }
  ...
  async update(
    ...
    const result = await this.baseWorkoutRepository.update(
      id,
