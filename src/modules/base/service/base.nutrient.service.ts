...
export class BaseNutrientService {
  ...
  async create(
    createBaseNutrientDto: CreateBaseNutrientDto,
  ): Promise<BaseNutrientEntity> {
    const newNutrient = this.baseNutrientRepository.create(
  ...
  async findAll(
    ...
    const queryBuilder =
      this.baseNutrientRepository.createQueryBuilder("nutrient");

    if (createdByUserId) {
      queryBuilder.where("nutrient.createdByUserId = :createdByUserId", {
        createdByUserId,
      });
    }
  ...
  async update(
    ...
    const result = await this.baseNutrientRepository.update(
      id,
