...
export class ActivityAccessRequirementsCheckerService {
   ...
   * @returns 
   */
  async check(activityId: number, userId: number): Promise<ProgramAccessDecision> {
    ...
    const programAccessCheckResult = 
      await this.programAccessRequirementsCheckerService
        .check(activity.programId, userId);
  ...
  private async checkActivityAccessRequirements(activity: ProgramStepActivityEntity, userId: number): Promise<ProgramAccessDecision> {
    const activityAccessRequirements: ProgramAccessRequirements = {...activity};
    ...
    if (activity.gymId) {
      ...
      const userGymAccessStatus = await this.userGymAccessStatusService.getUserGymAccessStatus(userId, activity.gymId);
      
      // Check if user satisfies the gym access requirements of the activity
      const gymLevelAccessResolver = new GymLevelAccessResolver(activityAccessRequirements, userGymAccessStatus);
    ...
    const userProgramAccessStatus = await this.userProgramAccessStatusService.getUserProgramAccessStatus(userId, activity.programId);
    const isFollowing = await this.gymManagerFollowerService.isFollowing(userId, activity.ownerManagerId);
    userProgramAccessStatus.managerFollowerStatus = isFollowing ? { ...isFollowing } : null;

    const programAccessLevelResolver = new ProgramLevelAccessResolver(activityAccessRequirements, userProgramAccessStatus);
