...
import { BrowserPreviousFromStepActivityWorkingsessionService } from "./browser.previous.from.step-activity-workingsession.service";
...
export class BrowserPreviousFromStepActivityService {
  ...
  async browse(current: ProgramNavigationNode): Promise<ProgramNavigationNode> {
    let previousStepActivity =
      await this.programStepActivityService.findPrevious(current.programItemId);
    ...
    while (previousStepActivity) {
      ...
      const previousNavigationItem =
        await this.browserPreviousFromStepActivityWorkingsessionService.browse({
      ...
      const everStartedOrCompletedActivity =
      ...
      previousStepActivity = await this.programStepActivityService.findPrevious(
        previousStepActivity.id,
      );
    }
