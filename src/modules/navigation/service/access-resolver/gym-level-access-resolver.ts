...
export class GymLevelAccessResolver {
  ...
  checkGymMembershipStatus(): ProgramAccessDecision {
    ...
    if (!this.userGymAccessStatus.isMember) {
    ...
    if (!this.programAccessRequirements.authorizedMembershipPlanIds?.length) {
      ...
      for (const membershipStatus of this.userGymAccessStatus
        .membershipStatuses) {
        if (membershipStatus.userPlanExists && membershipStatus.paid) {
          ...
          if (
            ...
            hasPaidAnyMembershipPlan = true;
            access.reason =
              ProgramNavigationReasonEnum.isOnGymMembershipPlanTrial;
            break;
          } else {
            return {
              ok: false,
      ...
      for (const membershipStatus of this.userGymAccessStatus
        ...
        if (
          ...
          this.programAccessRequirements.authorizedMembershipPlanIds.includes(
            membershipStatus.gymMembershipPlanId,
          ) &&
