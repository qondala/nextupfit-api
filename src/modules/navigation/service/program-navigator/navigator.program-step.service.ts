...
export class NavigatorProgramStepService {
  ...
  async next(current: ProgramNavigationNode): Promise<ProgramNavigationNode> {
    ...
    const programStepActivity = await this.programStepActivityService.findFirst(current.programItemId);
    
    // If the step has no activity
    // Go to the next step of the program
    if (!programStepActivity) {
      next.reasonCannotNavigate = ProgramNavigationReasonEnum.programHasNoStepActivity;

      let nextStep = await this.programStepService.findNext(current.programItemId);
      while (nextStep) {
    ...
    const programAccessRequirementsCheckResult = await this.programAccessRequirementsCheckerService
      .check(programStepActivity.programId, current.userId);
